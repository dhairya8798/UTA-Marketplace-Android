package com.uta.market.data.source.repository
...
class AuthRepository(
	...
	override suspend fun checkEmailAndMobile(
		...
		try {
			...
			if (queryResult != null) {
				...
				Log.d(TAG, "Query result null $queryResult")
			}
		} catch (e: Exception) {
			Log.d(TAG, "Error while check email and password ${e.localizedMessage}")
	...
	override fun signInWithPhoneAuthCredential(
		...
		try {
			...
			Log.d(TAG, "Error during sign up ${e.localizedMessage}")
			makeErrToast("Some Error Occurred", context)
		}
	}
