package com.uta.market.ui.loginSignup
...
import com.uta.market.EMAIL_ERROR_TEXT
import com.uta.market.MOB_ERROR_TEXT
import com.uta.market.R
import com.uta.market.data.utils.SignUpErrors
import com.uta.market.databinding.FragmentSignupBinding
			...
			override fun onClick(v: View?) {
				...
				if (viewModel.errorStatus.value == SignUpViewErrors.NONE) {
					viewModel.signErrorStatus.observe(viewLifecycleOwner) {
						if (it == SignUpErrors.NONE || it == null) {
							Log.d("SignupFragment", "onClick: Signup error nnone ")

							val bundle = bundleOf("uData" to viewModel.userData.value)
							launchOtpActivity(getString(R.string.signup_fragment_label), bundle)
						} else {
							Log.d("gnupFragment", "onClick: Signup errors $it")
